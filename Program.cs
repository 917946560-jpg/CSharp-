...
#region 知识点二 如何申明变量
//变量  可以变化的容器  变量就是用来存储各种不同类型数值的 一个容器
//不同的 变量类型 可以存储不同类型的值
...
//3.浮点数（小数）
//float 存储7/8位有效数字 根据编译器不同 有效数字也可能不一样 有四舍五入
//有效数字 是从左到右从非0数开始计算有效数字1.01234567890f
//之所以要在后面加f 是因为C#中 申明的小数 默认是double类型 加f 是告诉系统 它是float类型
...
string str = "牵牛花的花语是什么——坚实的羁绊，以及——幻化无常的恋爱";
Console.WriteLine(str);

int x = 1000;
Console.WriteLine(x);
//变量的使用和修改 不能无中生有 必须先声明才能用
x = 900;
Console.WriteLine(x);
#endregion
#region 知识点三 为什么有那么多不同的变量类型
//不同的变量 存储的范围和类型不一样 本质是占用的内存空间不同
//选择的不同数据（变量）类型装在不同的数据
